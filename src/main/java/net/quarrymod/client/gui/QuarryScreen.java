...
public class QuarryScreen extends GuiBase<BuiltScreenHandler> {
    ...
    protected void drawForeground(DrawContext drawContext, final int mouseX, final int mouseY) {
        ...
        if (blockEntity.getMineAll()) {
            builder.drawText(drawContext, this, Text.translatable("§lNot Used"), 39, 25, 0);
        }
    ...
    protected void drawMouseoverTooltip(DrawContext drawContext, int mouseX, int mouseY) {
        ...
        if (isPointWithinBounds(28, 18, 80, 19, mouseX, mouseY)
            ...
            drawContext.drawTooltip(textRenderer ,Text.translatable("gui.quarrymod.quarry.filler_blocks"), mouseX, mouseY);
        }

        if (isPointWithinBounds(118, 18, 38, 19, mouseX, mouseY)
            && this.focusedSlot != null
            && !this.focusedSlot.hasStack()) {
            drawContext.drawTooltip(textRenderer, Text.translatable("gui.quarrymod.quarry.drill_tubes"), mouseX, mouseY);
        }

        if (isPointWithinBounds(-42, 30, 19, 38, mouseX, mouseY)
            && this.focusedSlot != null
            && !this.focusedSlot.hasStack()) {
            drawContext.drawTooltip(textRenderer, Text.translatable("gui.quarrymod.quarry.drill_upgrades"), mouseX, mouseY);
        }
